...
func NewCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "rethink",
		Short: "Start a RethinkDB instance",
		Long:  `Start a RethinkDB instance in a Docker container with an interactive client`,
		RunE:  run,
	}
...
func run(cmd *cobra.Command, args []string) error {
	...
	if dataDir != "./data" { // Only process if explicitly set
		...
		log.Printf("Starting RethinkDB manager with data directory: %s", absDataDir)
	} else {
		log.Println("Starting RethinkDB manager with ephemeral storage")
	}

	manager := db.NewRethinkManager(absDataDir)
	...
	log.Println("Starting RethinkDB client...")
