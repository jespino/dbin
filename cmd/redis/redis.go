...
func NewCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  `Start a Redis instance in a Docker container with an interactive redis-cli client`,
		RunE:  run,
	}
...
func run(cmd *cobra.Command, args []string) error {
	...
	if dataDir != "./data" { // Only process if explicitly set
		...
		log.Printf("Starting Redis manager with data directory: %s", absDataDir)
	} else {
		log.Println("Starting Redis manager with ephemeral storage")
	}

	manager := db.NewRedisManager(absDataDir)
	...
	log.Println("Starting redis-cli client...")
