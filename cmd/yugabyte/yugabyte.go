...
func NewCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "yugabyte",
		Short: "Start a YugabyteDB instance",
		Long:  `Start a YugabyteDB instance in a Docker container with an interactive ysqlsh client`,
		RunE:  run,
	}
...
func run(cmd *cobra.Command, args []string) error {
	...
	if dataDir != "./data" { // Only process if explicitly set
		...
		log.Printf("Starting YugabyteDB manager with data directory: %s", absDataDir)
	} else {
		log.Println("Starting YugabyteDB manager with ephemeral storage")
	}

	manager := db.NewYugabyteManager(absDataDir)
	...
	log.Println("Starting ysqlsh client...")
