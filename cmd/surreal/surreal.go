...
func NewCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "surreal",
		Short: "Start a SurrealDB instance",
		Long:  `Start a SurrealDB instance in a Docker container with an interactive SQL client`,
		RunE:  run,
	}
...
func run(cmd *cobra.Command, args []string) error {
	...
	if dataDir != "./data" { // Only process if explicitly set
		...
		log.Printf("Starting SurrealDB manager with data directory: %s", absDataDir)
	} else {
		log.Println("Starting SurrealDB manager with ephemeral storage")
	}

	manager := db.NewSurrealManager(absDataDir)
	...
	log.Println("Starting SurrealDB SQL client...")
