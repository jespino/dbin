...
func NewCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "mariadb",
		Short: "Start a MariaDB instance",
		Long:  `Start a MariaDB instance in a Docker container with an interactive mysql client`,
		RunE:  run,
	}
...
func run(cmd *cobra.Command, args []string) error {
	...
	if dataDir != "./data" { // Only process if explicitly set
		...
		log.Printf("Starting MariaDB manager with data directory: %s", absDataDir)
	} else {
		log.Println("Starting MariaDB manager with ephemeral storage")
	}

	manager := db.NewMariaDBManager(absDataDir)
	...
	log.Println("Starting mysql client...")
