...
func NewCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "cassandra",
		Short: "Start a Cassandra database instance",
		Long:  `Start a Cassandra database instance in a Docker container with an interactive cqlsh client`,
		RunE:  run,
	}
...
func run(cmd *cobra.Command, args []string) error {
	...
	if dataDir != "./data" { // Only process if explicitly set
		...
		log.Printf("Starting Cassandra manager with data directory: %s", absDataDir)
	} else {
		log.Println("Starting Cassandra manager with ephemeral storage")
	}

	manager := db.NewCassandraManager(absDataDir)
	...
	log.Println("Starting cqlsh client...")
