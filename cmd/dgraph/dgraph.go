...
func NewCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "dgraph",
		Short: "Start a Dgraph instance",
		Long:  `Start a Dgraph instance in a Docker container with an interactive Ratel client`,
		RunE:  run,
	}
...
func run(cmd *cobra.Command, args []string) error {
	...
	if dataDir != "./data" { // Only process if explicitly set
		...
		log.Printf("Starting Dgraph manager with data directory: %s", absDataDir)
	} else {
		log.Println("Starting Dgraph manager with ephemeral storage")
	}

	manager := db.NewDgraphManager(absDataDir)
	...
	log.Println("Starting Ratel client...")
