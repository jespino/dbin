...
func NewCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "clickhouse",
		Short: "Start a ClickHouse instance",
		Long:  `Start a ClickHouse instance in a Docker container with an interactive clickhouse-client`,
		RunE:  run,
	}
...
func run(cmd *cobra.Command, args []string) error {
	...
	if dataDir != "./data" { // Only process if explicitly set
		...
		log.Printf("Starting ClickHouse manager with data directory: %s", absDataDir)
	} else {
		log.Println("Starting ClickHouse manager with ephemeral storage")
	}

	manager := db.NewClickHouseManager(absDataDir)
