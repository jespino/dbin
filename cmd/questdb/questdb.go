...
func NewCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "questdb",
		Short: "Start a QuestDB instance",
		Long:  `Start a QuestDB instance in a Docker container with an interactive questdb-cli client`,
		RunE:  run,
	}
...
func run(cmd *cobra.Command, args []string) error {
	...
	if dataDir != "./data" { // Only process if explicitly set
		...
		log.Printf("Starting QuestDB manager with data directory: %s", absDataDir)
	} else {
		log.Println("Starting QuestDB manager with ephemeral storage")
	}

	manager := db.NewQuestDBManager(absDataDir)
	...
	log.Println("Starting questdb-cli client...")
